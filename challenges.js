...
function sayHello() {
  ...
  function charCount(str) {
    ...
    let countByLetter = strArray.reduce((acc, char) => {
  ...
  // to use for loop instead of reduce: 
  
  // function charCount(str){
  //   countingObj ={}
  //   for(let i  = 0; i < str.length; i++){
  //     countingObj[str[i]] ? (countingObj[str[i]] ++) : countingObj[str[i]] = 1;
  //   }
  //   return countingObj
  // }
  ...
  function isPalindrome(string){
    word = string.toLowerCase().split(' ').join('');
  ...
  //Pseudocode: 
  // create function called mumble that accepts string
  // turn string into array
  // get character
  // add character to variable (repeated # of times of position)
  // add dash
  // repeat 
  ...
  // get items out of array
  // put items from array in object
  // return object
  ...
  // define merge objects accepts number of arguments 
  // merge 2nd-nth objects into first
  ...
  // define function that accepts an array
  // two variables (highest priced, result)
  // for function to compare array to highest priced
  // if bigger set to highest priced
  // result  = index of highest priced 
  // return result 
  ...
  // create function accepts two arguments
  // create new array variable
  // iterate through given array
  // invoke callback function - pass in index and given array element
  // add what is returned by callback function to new array
  ...
  // define a function that accepts a single array
  // go through each index and evaluate if typeOf is array
  // if array, go into it
  // take out value in it
  // rinse and repeat
  // if index + 1 is undefined, return
  ...
  // define function that accepts a num
  // if it is less than one or not a whole number (use modulo to see if there is a remainder), return 
  // check if input % 2 to half of input === 0
  // if === 0, return false
  // if != 0, return true
  ...
  // is number less than one or not a whole number? return an array
  // if number is prime add to array and return array
  // for loop should start with 2
  // if num % i === 0 push into the array
  // repeat above until when all the numbers when multiplied together equal the given argument, 
  // or num & i != 0
  ...
  // create a function
  // check to see if the ends "match" each other
  // if not, check to see if the match is in the string
  // find out what position the matching char is at
  ...
  //       if(string[j] === fromCharCode(asciiCode)){
  //         win++
  //       }
  //     }
  //   }
  ...
  function totalTaskTime(queue, threads) {
    let cpuArr = new Array(threads).fill(0)
    while (queue.length) {
      cpuArr[cpuArr.indexOf(Math.min(...cpuArr))] += queue.shift()
    }
    return Math.max(...cpuArr)
  }
  function totalTaskTime(queue, threads) {
    let computeArray = new Array(threads).fill(0)
    while (queue.length) {
      let leastUtilizedThread = computeArray.indexOf(Math.min(...computeArray))
      let nextTask = queue.shift()
      computeArray[leastUtilizedThread] += nextTask
    }
    return Math.max(...computeArray)
  }